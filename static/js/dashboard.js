document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateCategoryButtons() {
        ...
        var categories = ['Rent', 'Transportation', 'Utilities', 'Groceries','Eating out','other']; // Define categories
...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("createGroupForm").addEventListener("submit", function(event) {
        ...
        .then(data => {
            ...
            document.getElementById("generatedUrl").innerText = data; 
            // Show the URL display area
            document.getElementById("urlDisplay").style.display = 'block'; 
            
            // Now, dynamically update the form action attribute to include the generated URL
            var groupForm = document.getElementById("joinGroupForm");
            groupForm.action = data; // Set the action attribute to the generated URL
            groupForm.addEventListener("submit", function(event) {
                event.preventDefault(); // Prevent default form submission

                fetch(data, { // Use the generated URL for the fetch request
                    method: "POST"
                })
                .then(response => {
                    // Check if response indicates successful navigation
                    if(response.redirected) {
                        window.location.href = response.url; // Redirect to the generated URL
                    } else {
